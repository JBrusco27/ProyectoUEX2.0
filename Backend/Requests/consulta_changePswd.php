            ...
            setcookie('codigo_verif', $codigo, $verifExpiry, '/');
                ...
                switch($column_number){
                    case 0:
                        ...
                        $mail->Body = '<html><head><style>.em-container{text-align:center; height:200px;} .em-container *{margin:40px; color:black;} </style></head><body class="em-container"><h1 style="color: black;">Verify to change password</h1><p>Click the link below to change your Viauy password:</p><a href="http://localhost/Frontend/Account/VerifyComplete/verifyComplete.php?verif_code='.$codigo.'&pswd_hash='.$password_usuario_hashed.'" style="background-color:black; border-radius:10px; color:white; padding:15px 30px; text-decoration:none; margin:25px; cursor:pointer;">Reset Password</a></body></html>';
                        $mail->send();
                        break;
                    case 1:
                        $mail->Subject = 'Cambiar contraseña | Viauy';
                        $mail->Body = '<html><head><style>.em-container{text-align:center; height:200px;} .em-container *{margin:40px; color:black;} </style></head><body class="em-container"><h1 style="color: black;">Verificar para cambiar la contraseña</h1><p>Haz click en el link de abajo para cambiar tu contraseña de Viauy:</p><a href="http://localhost/Frontend/Account/VerifyComplete/verifyComplete.php?verif_code='.$codigo.'&pswd_hash='.$password_usuario_hashed.'" style="background-color:black; border-radius:10px; color:white; padding:15px 30px; text-decoration:none; margin:25px; cursor:pointer;">Reset Password</a></body></html>';
                        $mail->send();
                        break;
                    case 2:
                        $mail->Subject = 'Alterar a senha | Viauy';
                        $mail->Body = '<html><head><style>.em-container{text-align:center; height:200px;} .em-container *{margin:40px; color:black;} </style></head><body class="em-container"><h1 style="color: black;">Verifique para alterar a senha </h1><p>Clique no link abaixo para alterar sua senha Viauy:</p><a href="http://localhost/Frontend/Account/VerifyComplete/verifyComplete.php?verif_code='.$codigo.'&pswd_hash='.$password_usuario_hashed.'" style="background-color:black; border-radius:10px; color:white; padding:15px 30px; text-decoration:none; margin:25px; cursor:pointer;">Reset Password</a></body></html>';
                        $mail->send();
                        break;
                }
                    $email_sended = true;
